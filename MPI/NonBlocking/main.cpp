...
int main(int argc, char* argv[]) {
    ...
    MPI_Request req[n_threads];
    MPI_Status status[n_threads];
    double ts = MPI_Wtime();
    //逐行作为主行元素，进行初等行变换
    for (int k = 0; k < N; k++) {
        //同步当前主行元素到所有进程
        for(int i = 0;i < n_threads; i++)
            MPI_Ibcast(A[k], N, MPI_FLOAT, k % n_threads, MPI_COMM_WORLD,&req[i]);

        //各进程将自己所负责的行完成行初等变换
        int j = k + 1;
        while (j % n_threads != myid) j ++;
        MPI_Wait(&req[myid], &status[myid]);
        for(;j < N;j += n_threads){
            A[k][j] = A[k][j] / A[k][k];
        }
